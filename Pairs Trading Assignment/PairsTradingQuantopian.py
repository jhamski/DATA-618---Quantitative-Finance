"""
Pairs Trading based on the Quantopian Video Tutorial.

Cointegration tests in R https://github.com/jhamski/DATA-618---Quantitative-Finance/blob/master/Pairs%20Trading%20Assignment/testing_cointegration.Rmd
"""
...
def initialize(context):
    ...
    context.stock_pairs = [sid(128), sid(17526)]
    # set_benchmark(context.y)

    context.num_pairs = len(context.stock_pairs)
    # strategy specific variables
    context.lookback = 10 # used for regression
...
def check_pair_status(context, data):
    ...
    prices = data.history(context.stock_pairs, 'price', 1, '1d')

    new_spreads = np.ndarray((context.num_pairs, 1))

    for i in range(context.num_pairs):

        stock_y = context.stock_pairs[0]
        stock_x = context.stock_pairs[1]
